...
const speakerBtnData = [
  {
    img_url: new URL("../assets/image/speaker_closed.png", import.meta.url)
      .href,
    text: "扬声器关闭",
  },
  {
    img_url: new URL("../assets/image/speaker_opened.png", import.meta.url)
      .href,
    text: "扬声器开启",
  },
];
...
const requestVideoBtnHandler = async () => {
  ...
  multimediaManager.sendCustomMessage(
    targetUsername.value,
    InformationTypes.VideoRequest,
    null,
    null
  );
...
const requestDesktopBtnHandler = async () => {
  ...
  multimediaManager.sendCustomMessage(
    targetUsername.value,
    InformationTypes.DesktopRequest,
    null,
    null
  );
};
...
const cancelRequestDesktopHandler = () => {
  ...
  multimediaManager.sendCustomMessage(
    targetUsername.value,
    InformationTypes.CancelDesktop,
    null,
    null
  );
...
const disconnectBtnHandler = () => {
  ...
  multimediaManager.sendCustomMessage(
    desktopPassiveUsername.value,
    InformationTypes.OwnerCloseDesktop,
    null,
    null
  );
  isMonitored.value = false;
...
const connectVideo = (destID) => {
  ...
  dynamicCameraConnector4Other.SetViewer(
...
const desktopResult = (flag) => {
  ...
  multimediaManager.sendCustomMessage(
    desktopPassiveUsername.value,
    InformationTypes.DesktopResult,
    [flag ? 1 : 0],
    ""
  );
  isPassiveRequestDesktop.value = false;
...
const connectDesktop = (targetUserID) => {
  ...
  dynamicDesktopConnector.ConnectEnded = (ownerID, res) => {
    if (res.ResultCode === 3) {
      ...
      multimediaManager.sendCustomMessage(
        targetUsername.value,
        InformationTypes.GuestCloseDesktop,
        null,
        null
      );
      closeDesktop(true);
...
onBeforeMount(() => {
  ...
  axios.post("videoChatOver").then((res) => {
    if (res.code == 200) {
      // message.info("退出面试")
    }
  });
...
<template>
  ...
  <!-- 发起视频请求对话框 -->
  <RequestDialog2Active
    :isShow="isActiveRequestVideo"
    type="video"
    @close="cancelRequestVideoHandler"
  />
  <!-- 接受视频请求对话框 -->
  ...
  <!-- 发起桌面请求对话框 -->
  <RequestDialog2Active
    :isShow="isActiveRequestDesktop"
    type="desktop"
    @close="cancelRequestDesktopHandler"
  />
  <!-- 接受桌面请求对话框 -->
  <RequestDialog2Passive
