...
axios
  .post("GetCompany", { user_id: userInformation.user_id })
  .then((res) => {
    if (res.code == 200) {
...
const upload = () => {
  message.info("上传文件中...");
  axios
    .post("uploadCompany", { introduction: valueHtml.value })
    .then((res) => {
...
const showMap2 = () => {
  isShowMap2.value = !isShowMap2.value;
};
</script>

<template>
  <a-button v-if="isShowMap2" danger ghost @click="showMap2()" class="close-map"
    >关闭地图</a-button
  >
  <a-button v-if="isShowMap" danger ghost @click="showMap()" class="close-map"
    >关闭地图</a-button
  >
